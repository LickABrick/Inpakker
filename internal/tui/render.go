...
func (m Model) shellView() string {
	...
	if m.workspace != nil {
		...
		if m.updateResult.Available && m.backgroundActivity() != "" && width >= 100 {
			right += " · Update v" + m.updateResult.LatestVersion + " available"
		}
	}
	...
	if activity := m.backgroundActivity(); activity != "" {
		workspaceRight = m.spinner.View() + " " + activity
...
func (m Model) pageView(width, height int) string {
	...
	case RouteAbout:
		v := m.detailViewport
		v.SetWidth(width)
		v.SetHeight(height - 1)
		v.SetContent(m.aboutView(width))
		return v.View() + scrollPosition(v)
	...
	case RouteApplication:
		...
		viewport.SetHeight(height - 1)
		viewport.SetContent(m.applicationView(width))
		return viewport.View() + scrollPosition(viewport)
...
func (m Model) applicationsView(width int) string {
	...
	output.WriteString("\n\n")
	if m.apps.searching || m.apps.search.Value() != "" {
	...
	tableView := m.apps.table.View()
	if width >= 92 {
		if app, ok := m.apps.selected(); ok {
			tableView = m.splitPreview(tableView, m.applicationPreview(app, (width-3)/2), width, max(3, m.height-12))
		}
	}
...
func (m Model) applicationView(width int) string {
	...
	heading := m.theme.PageTitle.Render(display) + "\n" + padBetween(m.theme.TextMuted.Render(app.App.Ref.Relative), status, width)
	...
	configLines := []string{
		...
		keyValue("Path", app.App.Ref.Relative, 18),
		keyValue("Group", app.Group, 18),
		keyValue("Source", inheritedLabel(app.App.Effective.SourceDirectory, app.App.Effective.SourceInherited), 18),
		keyValue("Setup file", cfg.SetupFile, 18),
		keyValue("Output", inheritedLabel(app.App.Effective.OutputDirectory, app.App.Effective.OutputInherited), 18),
		keyValue("Install command", cfg.InstallCommand, 18),
		keyValue("Uninstall command", cfg.UninstallCommand, 18),
	}
	configPanel := m.theme.Panel.Render(ansi.Wrap(strings.Join(configLines, "\n"), max(20, width-6), ""))
	panels := packagePanel + "\n\n" + configPanel
...
func (m Model) footerView(width int) string {
	...
	for _, b := range bindings {
		...
		h := b.Key.Help()
		candidate := append(append([]string{}, hints...), h.Key+" "+h.Desc)
		if ansi.StringWidth(strings.Join(candidate, " · ")) > width {
			continue
		}
		hints = candidate
		shown = append(shown, b.Key)
	}
	return m.help.ShortHelpView(shown)
}
...
func (m Model) modalView() string {
	...
	if m.picking {
		return m.dialog(m.pickerTitle(), m.picker.View())
	}
	var body string
	switch m.modal {
	case ModalPalette:
	...
	case ModalMessage, ModalResults:
		...
		body = m.modalViewport.View() + scrollPosition(m.modalViewport)
	case ModalLogs:
		m.prepareLogViewport()
		body = m.logViewport.View() + scrollPosition(m.logViewport)
...
func (m Model) helpContent() string {
	bindings := m.helpContext
	if len(bindings) == 0 {
		bindings = m.pageBindings()
	}
	return m.bindingHelp(bindings)
}
